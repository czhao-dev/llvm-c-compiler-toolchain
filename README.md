...
A small C toolchain built from scratch, one piece at a time: a preprocessor, a compiler, a linker, and a build tool.
...
Source flows left to right through the pipeline, and `build-tool` is the
Makefile-driven orchestration layer that can wire any of these stages into
recipes:
...
Listed in logical pipeline order — preprocess, then compile and link, with
`build-tool` last since it orchestrates the others rather than sitting in
the linear flow:
